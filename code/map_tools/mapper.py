...
conteo = df.group_by("Provincia","Código Postal").len().rename({"len": "frecuencia"})
...
print(df_conteo)
df_conteo.to_csv("datasets/conteo_municipios.csv", index=False)

m = folium.Map(location=[41.65, -4.72], zoom_start=7, tiles="CartoDB positron")

provincias = df_conteo["Provincia"].unique()

provincias = [
    p for p in provincias
    if p not in ("DESCONOCIDO", "EXTRANJERO", None, "", "NAN", "NULL")
...
# Guardar mapa
m.save("mapa_diagnosticos_españa.html")
