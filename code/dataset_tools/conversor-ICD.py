...
if __name__ == "__main__":
    print("Iniciando el proceso de conversion de diagnosticos...")
    print(diagnosticos_unidos.head())
    ...
    print("Lista de diagnosticos no procesados:")
    print(codigos_perdidos)
    with open("codigos_perdidos.csv", "w", newline="") as f:
        writer = csv.writer(f)
        for codigo in codigos_perdidos:
            writer.writerow([codigo])
    
    
    diagnosticos_finales = estandarizar_diagnosticos_icd10(dataset)
